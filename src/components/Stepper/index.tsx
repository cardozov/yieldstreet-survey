...
const Step = ({ value, checked }: StepType) => {
  ...
  return <>
    ...
    <span className={`h-1 w-full ${checked ? 'bg-primary-100 dark:bg-primary-200' : 'bg-gray-300 dark:bg-gray-600'} last:hidden`}></span>
  </>
}

const Stepper = () => {
  const { step } = useContext(SurveyContext)
  const stepperMap = Array(STEP_COUNT).fill(0).map((_,index) => index+1)
  return (
    <div className="flex items-center">
      {
        stepperMap.map((value) => <Step key={value} value={value} checked={value < step} />)
      }
    </div>
  )
}
