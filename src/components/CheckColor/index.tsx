...
const CheckColor = ({ color, style }: { color: string; style: string; }) => {
  ...
  const change = () => {
    if (data.favoriteColors?.includes(color))
      setChunk({ favoriteColors: [...(data.favoriteColors?.filter(c => c !== color) || [])]})
    else
      setChunk({ favoriteColors: [ ...(data.favoriteColors || []), color ]})
  }
  ...
  return (
      <div className={
        ...
        onClick={change}
      >
        {data.favoriteColors?.includes(color) && <CheckIcon className='h-4 w-4 -ml-5' />}
        {color}
      </div>
  )
}
